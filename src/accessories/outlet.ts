...
export class Outlet extends ComelitAccessory<OutletDeviceData> {
  ...
  protected initServices(): Service[] {
    ...
    this.outletService = new HomebridgeAPI.hap.Service.Outlet(
    ...
    this.outletService
      ...
      .on(
        ...
        async (yes: boolean, callback: Function) => {
  ...
  public update(data: OutletDeviceData) {
    ...
    this.outletService
      .getCharacteristic(Consumption)
      .updateValue(`${data.instant_power} W`);
    this.log(
      `Reporting consumption for ${data.descrizione}: ${data.instant_power}Wh`
    );
